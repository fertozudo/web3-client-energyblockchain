...
const getCurrentGasPrices = async () => {
  const urlprices = process.env.URL_PRICES
  let response = await axios.get(urlprices)
  let prices = {
...
const main = async () => {
  ...
  let details = {
    ...
    "nonce": nonce,
    "chainId": parseInt(process.env.CHAIN_ID) // EIP 155 chainId - mainnet: 1, rinkeby: 4
  }
  log (details)
   ...
   */
  const url = `${process.env.URL_SCAN}${transactionId}`
  log(url.cyan)
